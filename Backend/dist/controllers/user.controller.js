...
export const updateDetailsUser = async (req, res) => {
    try {
        ...
        const updatedUserData = await userModel.findByIdAndUpdate(userId, updateData, { new: true, select: "-password" } // never return the password so i use this algo
        );
        if (!updatedUserData) {
...
export const filterUsersData = async (req, res) => {
    try {
